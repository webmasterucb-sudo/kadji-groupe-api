...
export class AppartementVideService {
    ...
    async create(
        createAppartementVideDto: CreateAppartementVideDto,
    ): Promise<AppartementVide> {
        const createdAppartementVide = new this.appartementVideModel(
            createAppartementVideDto,
        );
        return createdAppartementVide.save();
    }

    async findAll(): Promise<AppartementVide[]> {
        return this.appartementVideModel.find().sort({ updatedAt: -1 }).limit(1200).exec();
    }
    ...
    async update(
        id: string,
        updateAppartementVideDto: UpdateAppartementVideDto,
    ): Promise<AppartementVide> {
    ...
    async remove(id: string): Promise<AppartementVide> {
        const deletedAppartementVide = await this.appartementVideModel
