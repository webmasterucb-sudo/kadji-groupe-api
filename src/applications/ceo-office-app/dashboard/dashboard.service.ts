...
export class DashboardService {
    constructor(
        @InjectModel(TravelTicket.name) private travelTicketModel: Model<TravelTicket>,
        @InjectModel(MissionExpense.name) private missionExpenseModel: Model<MissionExpense>,
        @InjectModel(HotelApartement.name) private hotelApartementModel: Model<HotelApartement>,
        @InjectModel(AppartementMeuble.name) private appartementMeubleModel: Model<AppartementMeuble>,
        @InjectModel(AppartementVide.name) private appartementVideModel: Model<AppartementVide>,
    ) { }
    ...
    async getDashboardData() {
        ...
        const totalMissionExpenses = missionExpenses.reduce((sum, expense) => sum + (expense.montant || 0), 0);

        // 4. Hôtels
        const activeHotels = await this.hotelApartementModel.countDocuments({ status: { $in: ['RESERVE', 'CONFIRME'] } });

        // 5. Appart. Vides
        const emptyApartments = await this.appartementVideModel.countDocuments();

        // 6. Appart. Meublés
        const furnishedApartments = await this.appartementMeubleModel.countDocuments({ status: 'CONFIRMED' });

        // 7. Dépenses Total (Hébergement)
        // Sum of Hotel + Meuble + Vide expenses
        const hotels = await this.hotelApartementModel.find();
        const totalHotelExpenses = hotels.reduce((sum, hotel) => sum + (hotel.totalAmount || 0), 0);

        const meubles = await this.appartementMeubleModel.find();
        const totalMeubleExpenses = meubles.reduce((sum, meuble) => sum + (meuble.totalAmount || 0), 0);
        ...
        const totalVideExpenses = vides.reduce((sum, vide) => sum + (vide.monthlyRent || 0), 0);

        const totalAccommodationExpenses = totalHotelExpenses + totalMeubleExpenses + totalVideExpenses;
