...
import { FournisseurBillet, FournisseurBilletDocument } from './entities/fournisseur-billet.entity';
...
export class FournisseurBilletService {
    ...
    async create(createDto: CreateFournisseurBilletDto): Promise<FournisseurBilletDocument> {
    ...
    async update(id: string, updateDto: UpdateFournisseurBilletDto): Promise<FournisseurBilletDocument> {
        const fournisseur = await this.fournisseurBilletModel.findById(id).exec();
    ...
    async remove(id: string): Promise<{ success: boolean; message: string }> {
        const result = await this.fournisseurBilletModel.findByIdAndDelete(id).exec();
