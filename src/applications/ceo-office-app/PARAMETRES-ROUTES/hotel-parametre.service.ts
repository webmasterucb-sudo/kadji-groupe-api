...
import { HotelParametre, HotelParametreDocument } from './entities/hotel-parametre.entity';
...
export class HotelParametreService {
    ...
    async findAll(): Promise<HotelParametreDocument[]> {
        return await this.hotelParametreModel
            .find()
            .sort({ createdAt: -1 })
            .exec();
    }
    ...
    async create(createDto: CreateHotelParametreDto): Promise<HotelParametreDocument> {
    ...
    async update(id: string, updateDto: UpdateHotelParametreDto): Promise<HotelParametreDocument> {
        const hotel = await this.hotelParametreModel.findById(id).exec();
