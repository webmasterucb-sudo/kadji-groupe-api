...
import { ValidateurEmail, ValidateurEmailDocument } from './entities/validateur-email.entity';
...
export class ValidateurEmailService {
    ...
    async findAll(): Promise<ValidateurEmailDocument[]> {
        let data = await this.validateurEmailModel
            .find()
            .sort({ createdAt: -1 })
            .exec();
        return data;
    ...
    async create(createDto: CreateValidateurEmailDto): Promise<ValidateurEmailDocument> {
        const existingEmail = await this.validateurEmailModel
            .findOne({ email: createDto.email })
            .exec();

        if (existingEmail) {
            throw new ConflictException('Cet email est déjà enregistré comme validateur');
        }
    ...
    async update(id: string, updateDto: UpdateValidateurEmailDto): Promise<ValidateurEmailDocument> {
        const validateur = await this.validateurEmailModel.findById(id).exec();
