...
import { HotelApartement, HotelApartementDocument } from './entities/hotel-apartement.entity';
...
export class HotelApartementService {
    constructor(
        @InjectModel(HotelApartement.name) private hotelApartementModel: Model<HotelApartementDocument>,
    ) { }
    ...
    async update(id: string, updateHotelApartementDto: UpdateHotelApartementDto): Promise<HotelApartement> {
    ...
    async remove(id: string): Promise<HotelApartement> {
        const deletedHotelApartement = await this.hotelApartementModel.findByIdAndDelete(id).exec();
