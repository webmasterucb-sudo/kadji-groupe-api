...
export class AppartementMeubleService {
    ...
    async create(
        createAppartementMeubleDto: CreateAppartementMeubleDto,
    ): Promise<AppartementMeuble> {
        const createdAppartementMeuble = new this.appartementMeubleModel(
    ...
    async findAll(): Promise<AppartementMeuble[]> {
        return this.appartementMeubleModel.find().sort({ updatedAt: -1 }).limit(1200).exec();
    }

    async findOne(id: string): Promise<AppartementMeuble> {
        const appartementMeuble = await this.appartementMeubleModel
            .findById(id)
            .exec();
    ...
    async update(
        id: string,
        updateAppartementMeubleDto: UpdateAppartementMeubleDto,
    ): Promise<AppartementMeuble> {
    ...
    async remove(id: string): Promise<AppartementMeuble> {
        const deletedAppartementMeuble = await this.appartementMeubleModel
