...
export class UsersService {
    constructor(@InjectModel(AdminUser.name) private userModel: Model<AdminUser>) { }
    ...
    async updateOne(userId: string, updateUserDto: any): Promise<AdminUser | null> {
        return this.userModel.findByIdAndUpdate(userId, updateUserDto).exec();
    }

    async onChangeStatutCompte(userId: string, statutCompte: string): Promise<AdminUser | null> {
